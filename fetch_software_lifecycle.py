"""Fetch Fortinet Software RSS lifecycle feed and split entries into one JSON file per product.

Unlike the Hardware feed, Software items have no explicit Category field, so
the product name is derived from the title (everything except the trailing
version number, e.g. "FortiADC Manager 6.2" -> "FortiADC Manager").

Newly added entries (not previously present in the product JSON files) are
reported to a Discord webhook, one message per entry.

Usage:
    python fetch_software_lifecycle.py [--url URL] [--out-dir DIR] [--webhook-url URL]
"""
...
DEFAULT_URL = "https://support.fortinet.com/rss/Software.xml"
DEFAULT_OUT_DIR = "categories_software"
DEFAULT_STATE_DIR = "state_software"
DISCORD_COLOR = 0x3498DB
DISCORD_ORANGE = 0xE67E22
DISCORD_RED = 0xE74C3C
DATE_FIELDS = ("End of Engineering Support", "End of Support")
...
def derive_product(title: str) -> str:
    """Strip the trailing version token from a title, e.g. "FortiADC 7.2" -> "FortiADC"."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir", default=DEFAULT_OUT_DIR, help="Directory to write per-product JSON files"
    )
    parser.add_argument(
        "--webhook-url",
